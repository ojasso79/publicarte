...
$info_right_column .= '<span style="font-weight:bold;font-size:27px;">ORDEN DE TRABAJO</span><br />';
...
if (!empty($estimate->client->vat)) {
    $client_details .= '<br />RFC: ' . $estimate->client->vat;
}
...
// Dates
$pdf->Cell(0, 0, 'Fecha de la orden: ' . _d($estimate->date), 0, 1, ($swap == '1' ? 'L' : 'R'), 0, '', 0);
if (!empty($estimate->expirydate)) {
    $pdf->Cell(0, 0, 'Fecha de entrega: ' . _d($estimate->expirydate), 0, 1, ($swap == '1' ? 'L' : 'R'), 0, '', 0);
}
...
if($estimate->sale_agent != 0){
    if(get_option('show_sale_agent_on_estimates') == 1){
       $pdf->Cell(0, 0, 'Vendedor: ' .  get_staff_full_name($estimate->sale_agent), 0, 1, ($swap == '1' ? 'L' : 'R'), 0, '', 0);
   }
}
...
<tr height="30" bgcolor="'.get_option('pdf_table_heading_color').'" style="color:'.get_option('pdf_table_heading_text_color').';">
    ...
    <th width="'.$item_width.'%" align="left">Artīculo</th>
    <th width="12%" align="right">Cantidad</th>
    <th width="15%" align="right">Precio</th>';
    if(get_option('show_tax_per_item') == 1){
        $tblhtml .= '<th width="15%" align="right">IVA</th>';
    }
    $tblhtml .='<th width="15%" align="right">Monto</th>
</tr>';
// Items
...
//************************* aqui finaliza la tabla de desglose de articulos
